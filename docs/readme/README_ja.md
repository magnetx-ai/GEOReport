...
| ローカル構築をしない | [ホスト版レポート](#ホスト版レポート) | 同じ診断を、Python・キー・ローカルサーバーなしで実行 |
...
このリポジトリは、AI 検索可視性を分析する中核機能を提供します。ローカル環境を用意せずにフルの GEO 診断を走らせたい場合は、ホスト版で無料レポートを生成できます。MagUp が結果の読み解きも行います。

[無料の GEO レポートを生成](https://console.magup.ai/survey?templateId=6a478e309d2f99db4ce05590)
